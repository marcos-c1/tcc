	...
	if [ $group == 'cd-box' ]; then 
		case $method in
			'fourier_real')
				...
				cat $CD_BOX_NEGATIVE_FOURIER_REAL_DIR/*.csv $CD_BOX_FOURIER_REAL_DIR/*.csv > $file 
				;;
			'fourier_zcurve')
				file='./Fourier_ZCurve/cd_box_fourier_zcurve_data.csv'
				cat $CD_BOX_NEGATIVE_FOURIER_ZCURVE_DIR/*.csv $CD_BOX_FOURIER_ZCURVE_DIR/*.csv > $file 
				;;
			'entropy_shannon')
				file='./Entropy/Shannon/cd_box_entropy_shannon_data.csv'
				cat $CD_BOX_NEGATIVE_ENTROPY_SHANNON_DIR/*.csv $CD_BOX_ENTROPY_SHANNON_DIR/*.csv > $file 
				;;
			'entropy_tsallis')
				file='./Entropy/Tsallis/cd_box_entropy_tsallis_data.csv'
				cat $CD_BOX_NEGATIVE_ENTROPY_TSALLIS_DIR/*.csv $CD_BOX_ENTROPY_TSALLIS_DIR/*.csv > $file 
				;;
			'complex')
				file='./Complex/cd_box_complex_data.csv'
				cat $CD_BOX_NEGATIVE_COMPLEX_DIR/*.csv $CD_BOX_COMPLEX_DIR/*.csv > $file 
				;;
			*)
		...
		case $method in
			'fourier_real')
				...
				cat $HACA_BOX_NEGATIVE_FOURIER_REAL_DIR/*.csv $HACA_BOX_FOURIER_REAL_DIR/*.csv > $file 
				;;
			'fourier_zcurve')
				file='./Fourier_ZCurve/haca_box_fourier_zcurve_data.csv'
				cat $HACA_BOX_NEGATIVE_FOURIER_ZCURVE_DIR/*.csv $HACA_BOX_FOURIER_ZCURVE_DIR/*.csv > $file 
				;;
			'entropy_shannon')
				file='./Entropy/Shannon/haca_box_entropy_shannon_data.csv'
				cat $HACA_BOX_NEGATIVE_ENTROPY_SHANNON_DIR/*.csv $HACA_BOX_ENTROPY_SHANNON_DIR/*.csv > $file 
				;;
			'entropy_tsallis')
				file='./Entropy/Tsallis/haca_box_entropy_tsallis_data.csv'
				cat $HACA_BOX_NEGATIVE_ENTROPY_TSALLIS_DIR/*.csv $HACA_BOX_ENTROPY_TSALLIS_DIR/*.csv > $file 
				;;
			'complex')
				file='./Complex/haca_box_complex_data.csv'
				cat $HACA_BOX_NEGATIVE_COMPLEX_DIR/*.csv $HACA_BOX_COMPLEX_DIR/*.csv > $file 
				;;
			*)
				echo -e "Método de extração de features não reconhecido."
				usage
